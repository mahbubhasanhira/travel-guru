		...
		<>
			...
			<div className="hotel_details_container container d-flex justify-content-between">
				<div className="hotel_card_container col-md-7 col-lg-7 col-sm-12">
					...
					{hotelDetails.map((singleDetail, index) => (
						<div
							key={index}
							className="hotel_details d-flex align-items-center justify-content-between"
						>
							...
							<div className="col-lg-6 hotelDetails_data">
								...
								<p className="bedrooms">
									<span>{singleDetail.guest} guest</span>{" "}
									<span>{singleDetail.bedrooms} bedrooms</span>{" "}
									<span>{singleDetail.beds} beds</span> <span></span>
									{singleDetail.baths} baths
								</p>
								<p className="hotel_description">{singleDetail.description}</p>
								<p className="review_and_price">
									<FontAwesomeIcon style={{ color: "gold" }} icon={faStar} />
									<span>{singleDetail.rating}</span>{" "}
									<span>${singleDetail.price}/</span>night $
									{singleDetail.totalPrice} total
								</p>
							</div>
						</div>
					))}
